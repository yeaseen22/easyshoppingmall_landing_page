...
const inputClass = "w-full bg-[#1c2128] border border-gray-700 rounded-xl px-4 py-3.5 text-accent-content placeholder-gray-500 focus:outline-none focus:border-primary-color transition-colors text-sm";
...
export default function ReviewForm() {
  ...
  const {
    ...
    defaultValues: { customerName: "", customerEmail: "", rating: 0, comment: "" },
  });
  ...
  return (
    <div className="bg-[#11151c] border border-accent-content/5 rounded-2xl p-6 md:p-8">
      <h3 className="text-xl font-bold text-accent-content mb-2">Leave a Review</h3>
      <p className="text-gray-400 text-sm mb-6">Share your experience with our products</p>

      {submitState === "success" ? (
        <div className="flex flex-col items-center justify-center py-8 text-center">
          <CheckCircle className="w-12 h-12 text-green-500 mb-3" />
          <p className="text-green-500 font-bold text-lg">Thank you!</p>
          <p className="text-gray-400 text-sm">Your review has been submitted and will appear after moderation.</p>
        </div>
      ) : submitState === "error" ? (
        <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              {[1, 2, 3, 4, 5].map((star) => (
                <button
                  ...
                  className={`p-1 transition-all hover:scale-110 ${
                    star <= currentRating ? "text-primary-color" : "text-gray-700"
                  }`}
                  aria-label={`${star} star${star > 1 ? "s" : ""}`}
                >
                  <Star size={28} fill={star <= currentRating ? "currentColor" : "none"} />
                </button>
              ))}
            </div>
            {errors.rating && (
