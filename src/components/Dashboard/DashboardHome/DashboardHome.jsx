...
import { getDashboardStats, getRecentActivity } from "@/action/dashboard";
...
function StatCard({ icon, label, value, color, isLoading }) {
  ...
  return (
    <div className="bg-[#11151c] p-7 rounded-2xl border border-accent-content/5 shadow-xl hover:border-[#d4af37]/30 transition-all group">
      <div className={`mb-5 p-3 bg-accent-content/5 w-fit rounded-xl group-hover:scale-110 transition-transform ${color}`}>
        {icon}
      </div>
  ...
  const quickStats = [
    {
      id: 1,
      name: "Total Revenue",
      value: stats ? `৳${(stats.totalRevenue || 0).toLocaleString()}` : `৳${totalRevenue}`,
            ...
            {isLoading ? (
              ...
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={stats?.salesTrend || []}>
                  ...
                  <YAxis tick={{ fill: "#6b7280", fontSize: 10 }} />
                  <Tooltip content={<CustomTooltip />} />
                  <Bar dataKey="orders" fill={COLORS.primary} radius={[4, 4, 0, 0]} name="Orders" />
                </BarChart>
              </ResponsiveContainer>
            )}
          </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:items-center bg-[#080808] p-4 rounded-xl border border-accent-content/5">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-3 mt-2 text-xs text-gray-400">
                      <p>
                        Qty: <span className="text-accent-content">{selectedOrder.quantity}</span>
                      </p>
                      <p>
                        Price:{" "}
                        <span className="text-[#d4af37]">৳{selectedOrder.totalPrice}</span>
                      </p>
                    </div>
                  </div>
                </div>
