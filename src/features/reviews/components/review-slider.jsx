...
export default function ReviewSlider({ reviews = [] }) {
  return (
    ...
    >
      {reviews.length === 0 ? (
        <>
          <div className="text-muted-foreground text-2xl font-bold flex items-center justify-center h-full">
        ...
        <>
          <CarouselContent className="items-stretch">
            {reviews.map((review, i) => (
              <CarouselItem
                key={review._id}
                className="basis-full sm:basis-1/2 md:basis-full xl:basis-1/2 h-full"
              >
                <ReviewCard
                  review={review}
