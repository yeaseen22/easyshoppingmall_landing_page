    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8">
      <div className="bg-[#11151c] rounded-2xl shadow-xl border border-accent-content/5 p-6 md:p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className={labelClass}>Banner Title (HTML Allowed)</label>
            <p className="text-xs text-gray-500 mb-2">
              Example: Shop &lt;span class=&quot;text-transparent bg-clip-text bg-linear-to-r from-primary-color to-orange-400&quot;&gt;Smarter&lt;/span&gt;,&lt;br /&gt; Save Big.
            </p>
            <textarea value={title} onChange={(e) => setTitle(e.target.value)} rows={4} className={inputClass} placeholder="Enter hero banner title..." required />
          </div>

          <div>
            <label className={labelClass}>Banner Description</label>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className={inputClass} placeholder="Experience the future of online shopping..." required />
          </div>

          <div>
            <label className={labelClass}>Banner Image URL</label>
            <input type="url" value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} className={inputClass} placeholder="https://example.com/image.jpg" required />
          </div>
          ...
          {imageUrl && (
            <div>
              ...
              <div className="relative w-full h-64 rounded-xl overflow-hidden border border-accent-content/10 mt-2 bg-black/50">
                <Image src={imageUrl} alt="Banner preview" className="w-full h-full object-cover" width={800} height={400} onError={(e) => { e.target.style.display = 'none'; }} />
              </div>
            </div>
          )}
